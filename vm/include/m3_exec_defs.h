...
#define d_m3Op(NAME)                                                           \
    M3_NO_UBSAN d_m3RetSig op_##NAME(d_m3OpSig, cstr_t i_operationName)
...
#define nextOpImplGas() forceexit?m3Err_trapKilled:(++gascnt, ((IM3Operation)(*_pc))(_pc + 1, d_m3OpArgs))
...
#define nextOpDirect()                                                         \
    if (not forceexit) {                                                       \
        ...
        return m3Err_trapKilled;
#define jumpOpDirect(PC)                                                       \
    if (not forceexit) {                                                       \
        ++gascnt;                                                              \
        M3_MUSTTAIL return jumpOpImpl((pc_t)(PC));                             \
    } else                                                                     \
        return m3Err_trapKilled;
