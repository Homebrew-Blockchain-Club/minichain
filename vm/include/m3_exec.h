...
// TODO: all these functions could move over to the .c at some point. normally,
// I'd say screw it, but it might prove useful to be able to compile m3_exec
// alone w/ optimizations while the remaining code is at debug O0

// About the naming convention of these operations/macros (_rs, _sr_, _ss, _srs,
// etc.)
//------------------------------------------------------------------------------------------------------
//   - 'r' means register and 's' means slot
//   - the first letter is the top of the stack
//
//  so, for example, _rs means the first operand (the first thing pushed to the
//  stack) is in a slot and the second operand (the top of the stack) is in a
//  register
//------------------------------------------------------------------------------------------------------
...
#define fillBacktraceFrame(FUNCTION)                                           \
    (FillBacktraceFunctionInfo(_mem->runtime, function))
...
#define d_m3TraceLoad(TYPE, offset, val)                                       \
    d_m3TracePrint("load." #TYPE "  0x%x = %" PRI##TYPE, offset, val)
#define d_m3TraceStore(TYPE, offset, val)                                      \
    d_m3TracePrint("store." #TYPE " 0x%x , %" PRI##TYPE, offset, val)
#else
#define d_m3TraceLoad(TYPE, offset, val)
#define d_m3TraceStore(TYPE, offset, val)
#endif
...
#define d_outOfBounds                                                          \
    newTrap(ErrorRuntime(m3Err_trapOutOfBoundsMemoryAccess, _mem->runtime,     \
                         "memory size: %zu; access offset: %zu", _mem->length, \
                         operand))

#define d_outOfBoundsMemOp(OFFSET, SIZE)                                       \
    newTrap(ErrorRuntime(m3Err_trapOutOfBoundsMemoryAccess, _mem->runtime,     \
                         "memory size: %zu; access offset: %zu; size: %u",     \
...
// Accept macros
#define d_m3CommutativeOpMacro_i(TYPE, NAME, MACRO, ...)                       \
    d_m3CommutativeOpMacro(_r0, _r0, TYPE, NAME, MACRO, ##__VA_ARGS__)
#define d_m3OpMacro_i(TYPE, NAME, MACRO, ...)                                  \
    d_m3OpMacro(_r0, _r0, TYPE, NAME, MACRO, ##__VA_ARGS__)
#define d_m3CommutativeOpMacro_f(TYPE, NAME, MACRO, ...)                       \
    d_m3CommutativeOpMacro(_fp0, _fp0, TYPE, NAME, MACRO, ##__VA_ARGS__)
#define d_m3OpMacro_f(TYPE, NAME, MACRO, ...)                                  \
    d_m3OpMacro(_fp0, _fp0, TYPE, NAME, MACRO, ##__VA_ARGS__)

#define M3_FUNC(RES, A, B, OP)                                                 \
    (RES) = OP((A), (B)) // Accept functions: res = OP(a,b)
#define M3_OPER(RES, A, B, OP)                                                 \
    (RES) = ((A)OP(B)) // Accept operators: res = a OP b

#define d_m3CommutativeOpFunc_i(TYPE, NAME, OP)                                \
    d_m3CommutativeOpMacro_i(TYPE, NAME, M3_FUNC, OP)
#define d_m3OpFunc_i(TYPE, NAME, OP) d_m3OpMacro_i(TYPE, NAME, M3_FUNC, OP)
#define d_m3CommutativeOpFunc_f(TYPE, NAME, OP)                                \
    d_m3CommutativeOpMacro_f(TYPE, NAME, M3_FUNC, OP)
#define d_m3OpFunc_f(TYPE, NAME, OP) d_m3OpMacro_f(TYPE, NAME, M3_FUNC, OP)

#define d_m3CommutativeOp_i(TYPE, NAME, OP)                                    \
    d_m3CommutativeOpMacro_i(TYPE, NAME, M3_OPER, OP)
#define d_m3Op_i(TYPE, NAME, OP) d_m3OpMacro_i(TYPE, NAME, M3_OPER, OP)
#define d_m3CommutativeOp_f(TYPE, NAME, OP)                                    \
    d_m3CommutativeOpMacro_f(TYPE, NAME, M3_OPER, OP)
...
#define d_m3CompareOp_f(TYPE, NAME, OP)                                        \
    d_m3OpMacro(_r0, _fp0, TYPE, NAME, M3_OPER, OP)
#define d_m3CommutativeCmpOp_f(TYPE, NAME, OP)                                 \
    d_m3CommutativeOpMacro(_r0, _fp0, TYPE, NAME, M3_OPER, OP)

//-----------------------

// signed
d_m3CommutativeOp_i(i32, Equal, ==) d_m3CommutativeOp_i(i64, Equal, ==)
    d_m3CommutativeOp_i(i32, NotEqual, !=)
        d_m3CommutativeOp_i(i64, NotEqual, !=)
            ...
            d_m3Op_i(i32, LessThan, <) d_m3Op_i(i64, LessThan, <)
                d_m3Op_i(i32, GreaterThan, >) d_m3Op_i(i64, GreaterThan, >)
                    d_m3Op_i(i32, LessThanOrEqual, <=)
                        d_m3Op_i(i64, LessThanOrEqual, <=)
                            d_m3Op_i(i32, GreaterThanOrEqual, >=)
                                d_m3Op_i(i64, GreaterThanOrEqual, >=)

    // unsigned
    d_m3Op_i(u32, LessThan, <) d_m3Op_i(u64, LessThan, <) d_m3Op_i(
        u32, GreaterThan,
        >) d_m3Op_i(u64, GreaterThan,
                    >) d_m3Op_i(u32, LessThanOrEqual,
                                <=) d_m3Op_i(u64, LessThanOrEqual,
                                             <=) d_m3Op_i(u32,
                                                          GreaterThanOrEqual,
                                                          >=) d_m3Op_i(u64,
                                                                       GreaterThanOrEqual,
...
#if d_m3HasFloat
        d_m3CommutativeCmpOp_f(f32, Equal, ==) d_m3CommutativeCmpOp_f(
            f64, Equal,
            ==) d_m3CommutativeCmpOp_f(f32, NotEqual,
                                       !=) d_m3CommutativeCmpOp_f(f64, NotEqual,
                                                                  !=)
            d_m3CompareOp_f(f32, LessThan, <) d_m3CompareOp_f(
                f64, LessThan,
                <) d_m3CompareOp_f(f32, GreaterThan,
                                   >) d_m3CompareOp_f(f64, GreaterThan, >)
                d_m3CompareOp_f(f32, LessThanOrEqual, <=) d_m3CompareOp_f(
                    f64, LessThanOrEqual,
...
#define OP_MUL_64(A, B) (i64)((u64)(A) * (u64)(B))
                    ...
                    d_m3CommutativeOpFunc_i(
                        i32, Add, OP_ADD_32) d_m3CommutativeOpFunc_i(i64, Add,
                                                                     OP_ADD_64)
                        d_m3CommutativeOpFunc_i(
                            i32, Multiply,
                            OP_MUL_32) d_m3CommutativeOpFunc_i(i64, Multiply,
                                                               OP_MUL_64)

                            d_m3OpFunc_i(i32, Subtract,
                                         OP_SUB_32) d_m3OpFunc_i(i64, Subtract,
                                                                 OP_SUB_64)
...
#define OP_SHR_64(X, N) ((X) >> ((u64)(N) % 64))
                                ...
                                d_m3OpFunc_i(u32, ShiftLeft, OP_SHL_32) d_m3OpFunc_i(
                                    u64, ShiftLeft,
                                    OP_SHL_64) d_m3OpFunc_i(i32, ShiftRight,
                                                            OP_SHR_32)
                                    d_m3OpFunc_i(i64, ShiftRight, OP_SHR_64) d_m3OpFunc_i(
                                        u32, ShiftRight,
                                        OP_SHR_32) d_m3OpFunc_i(u64, ShiftRight, OP_SHR_64)

                                        d_m3CommutativeOp_i(u32, And, &) d_m3CommutativeOp_i(
                                            u32, Or,
                                            |) d_m3CommutativeOp_i(u32, Xor, ^)

                                            d_m3CommutativeOp_i(u64, And, &) d_m3CommutativeOp_i(
                                                u64, Or,
                                                |) d_m3CommutativeOp_i(u64, Xor,
                                                                       ^)

#if d_m3HasFloat
                                                d_m3CommutativeOp_f(f32, Add, +) d_m3CommutativeOp_f(
                                                    f64, Add,
                                                    +) d_m3CommutativeOp_f(f32, Multiply, *)
                                                    d_m3CommutativeOp_f(
                                                        f64,
                                                        Multiply,
                                                            *) d_m3Op_f(f32, Subtract, -)
                                                        d_m3Op_f(f64, Subtract, -) d_m3Op_f(
                                                            f32, Divide,
                                                            /) d_m3Op_f(f64, Divide, /)
#endif
...
#define d_m3UnaryOp_i(TYPE, NAME, OPERATION)                                   \
    d_m3UnaryMacro(_r0, _r0, TYPE, NAME, M3_UNARY, OPERATION)
#define d_m3UnaryOp_f(TYPE, NAME, OPERATION)                                   \
    d_m3UnaryMacro(_fp0, _fp0, TYPE, NAME, M3_UNARY, OPERATION)
...
// clz(0), ctz(0) results are undefined for rest platforms, fix it
    ...
    !(defined(__AVX2__) || (defined(__ABM__) && defined(__BMI__)))
...
d_m3UnaryOp_i(i32, Extend8_s, OP_EXTEND8_S_I32) d_m3UnaryOp_i(i32, Extend16_s,
                                                              OP_EXTEND16_S_I32)
    d_m3UnaryOp_i(i64, Extend8_s, OP_EXTEND8_S_I64) d_m3UnaryOp_i(
        i64, Extend16_s, OP_EXTEND16_S_I64) d_m3UnaryOp_i(i64, Extend32_s,
                                                          OP_EXTEND32_S_I64)
...
#if d_m3HasFloat
        d_m3TruncMacro(_r0, _fp0, i32, Trunc, f32,
                       OP_I32_TRUNC_F32) d_m3TruncMacro(_r0, _fp0, u32, Trunc,
                                                        f32, OP_U32_TRUNC_F32)
            d_m3TruncMacro(_r0, _fp0, i32, Trunc, f64,
                           OP_I32_TRUNC_F64) d_m3TruncMacro(_r0, _fp0, u32,
                                                            Trunc, f64,
                                                            OP_U32_TRUNC_F64)

                d_m3TruncMacro(
                    _r0, _fp0, i64, Trunc, f32,
                    OP_I64_TRUNC_F32) d_m3TruncMacro(_r0, _fp0, u64, Trunc, f32,
                                                     OP_U64_TRUNC_F32)
                    d_m3TruncMacro(
                        _r0, _fp0, i64, Trunc, f64,
                        OP_I64_TRUNC_F64) d_m3TruncMacro(_r0, _fp0, u64, Trunc,
                                                         f64, OP_U64_TRUNC_F64)

                        d_m3TruncMacro(_r0, _fp0, i32, TruncSat, f32,
                                       OP_I32_TRUNC_SAT_F32)
                            d_m3TruncMacro(_r0, _fp0, u32, TruncSat, f32,
                                           OP_U32_TRUNC_SAT_F32)
                                d_m3TruncMacro(_r0, _fp0, i32, TruncSat, f64,
                                               OP_I32_TRUNC_SAT_F64)
                                    d_m3TruncMacro(_r0, _fp0, u32, TruncSat,
                                                   f64, OP_U32_TRUNC_SAT_F64)

                                        d_m3TruncMacro(_r0, _fp0, i64, TruncSat,
                                                       f32,
                                                       OP_I64_TRUNC_SAT_F32)
                                            d_m3TruncMacro(_r0, _fp0, u64,
                                                           TruncSat, f32,
                                                           OP_U64_TRUNC_SAT_F32)
                                                d_m3TruncMacro(
                                                    _r0, _fp0, i64, TruncSat,
                                                    f64, OP_I64_TRUNC_SAT_F64)
                                                    d_m3TruncMacro(
                                                        _r0, _fp0, u64,
                                                        TruncSat, f64,
...
#if d_m3HasFloat
d_m3ReinterpretOp(_r0, i32, _fp0, f32) d_m3ReinterpretOp(_r0, i64, _fp0, f64)
    ...
    slot(u32) =
        *global; //  printf ("get global: %p %" PRIi64 "\n", global, *global);
...
d_m3Op(SetGlobal_i32) {
    u32 *global = immediate(u32 *);
    *global =
        (u32)_r0; //  printf ("set global: %p %" PRIi64 "\n", global, _r0);
...
d_m3Op(SetGlobal_i64) {
    u64 *global = immediate(u64 *);
    *global =
        (u64)_r0; //  printf ("set global: %p %" PRIi64 "\n", global, _r0);
...
d_m3Op(CallRawFunction) {
    ...
    outp +=
        snprintf(outp, oute - outp, "%s!%s(", ctx.function->import.moduleUtf8,
                 ctx.function->import.fieldUtf8);
    ...
    for (int i = 0; i < nArgs; i++) {
        ...
        switch (type) {
            case c_m3Type_i32:
                outp +=
                    snprintf(outp, oute - outp, "%" PRIi32, *(i32 *)(args + i));
                break;
            case c_m3Type_i64:
                outp +=
                    snprintf(outp, oute - outp, "%" PRIi64, *(i64 *)(args + i));
                break;
            case c_m3Type_f32:
                outp +=
                    snprintf(outp, oute - outp, "%" PRIf32, *(f32 *)(args + i));
                break;
            case c_m3Type_f64:
                outp +=
                    snprintf(outp, oute - outp, "%" PRIf64, *(f64 *)(args + i));
                break;
            default:
...
// it's a debate: should the compilation be trigger be the caller or callee
// page. it's a much easier to put it in the caller pager. if it's in the
// callee, either the entire page has be left dangling or it's just a stub that
// jumps to a newly acquired page.  In Gestalt, I opted for the stub approach.
// Stubbing makes it easier to dynamically free the compilation. You can also do
// both.
d_m3Op(Compile) {
    rewrite_op(op_Call);
    ...
    if (M3_UNLIKELY(
            not function->compiled)) // check to see if function was compiled
                                     // since this operation was emitted.
...
#if d_m3EnableStrace >= 2
        d_m3TracePrint(
            "%s %s {", m3_GetFunctionName(function),
...
d_m3Op(Loop) {
    ...
    do {
        ...
        d_m3TracePrint("}");
#endif
        // linear memory pointer needs refreshed here because the block it's
        // looping over can potentially invoke the grow operation.
        _mem = memory->mallocated;
    } while (r == _pc);
...
d_m3Op(ContinueLoop) {
    ...
    // TODO: this is where execution can "escape" the M3 code and callback to
    // the client / fiber switch OR it can go in the Loop operation. I think
    // it's best to do here. adding code to the loop operation has the potential
    // to increase its native-stack usage. (don't forget ContinueLoopIf too.)
...
d_m3Op(Unsupported) { newTrap("unsupported instruction executed"); }
...
#define d_m3Store_f(SRC_TYPE, DEST_TYPE)                                       \
    d_m3Store(_fp0, SRC_TYPE, DEST_TYPE) d_m3StoreFp(_fp0, SRC_TYPE);
...
d_m3Store_f(f32, f32) d_m3Store_f(f64, f64)
    ...
    d_m3Store_i(i32, u8) d_m3Store_i(i32, i16) d_m3Store_i(i32, i32)
        ...
        d_m3Store_i(i64, u8) d_m3Store_i(i64, i16) d_m3Store_i(i64, i32)
            d_m3Store_i(i64, i64)
